...
export function ScoreHistoryChart({ sessions }: { sessions: SessionSummary[] }) {
  const completed = sessions.filter((s) => s.score_pct !== null && s.finished_at !== null).slice(0, 20).reverse()
  ...
  const data = completed.map((s, i) => ({ label: `#${i + 1}`, score: s.score_pct ?? 0, exam: s.exam_type }))
    ...
    <ResponsiveContainer width="100%" height={180}>
      <LineChart data={data} margin={{ top: 8, right: 16, bottom: 0, left: -16 }}>
        ...
        <Tooltip
          ...
          labelFormatter={(label, payload) => payload?.[0] ? `${label} — ${(payload[0].payload as { exam: string }).exam}` : label}
        />
        <ReferenceLine y={80} stroke="#22c55e" strokeDasharray="4 4" strokeWidth={1.5} />
        <Line type="monotone" dataKey="score" stroke="#0d8be8" strokeWidth={2.5} dot={{ r: 4, fill: '#0d8be8', strokeWidth: 0 }} activeDot={{ r: 6, fill: '#026dc6' }} />
      </LineChart>
    </ResponsiveContainer>
  )
}
