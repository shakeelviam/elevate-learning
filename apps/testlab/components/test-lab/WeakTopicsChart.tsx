...
export function WeakTopicsChart({ topics }: { topics: WeakTopic[] }) {
  if (topics.length === 0) {
    return (
      <div className="flex h-32 items-center justify-center rounded-xl bg-gray-50 text-sm text-gray-400">
        No data yet — complete a session to start
      </div>
    )
  }

  const data = topics.slice(0, 8).map((t) => ({ topic: t.topic, misses: t.miss_count })).reverse()
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(140, data.length * 36)}>
      <BarChart layout="vertical" data={data} margin={{ top: 0, right: 16, bottom: 0, left: 0 }}>
        ...
        <YAxis type="category" dataKey="topic" width={120} tick={{ fontSize: 12, fill: '#475569' }} axisLine={false} tickLine={false} />
        <Tooltip
          cursor={{ fill: '#f8fafc' }}
          contentStyle={{ borderRadius: 10, border: '1px solid #e2e8f0', fontSize: 12, boxShadow: '0 4px 16px rgba(0,0,0,0.08)' }}
          formatter={(val) => [Number(val), 'misses']}
        />
